...
**c. Baseline sanity.** Compare the model's baseline-arm `expected_reward_mean` against the *recent* holdout conversion rate. If they disagree by a meaningful fraction of the claimed lift, the modeled figures are inflating from a stale denominator (mechanism in [reading-the-model.md](references/reading-the-model.md)) — present the holdout-denominated figure alongside and do not let the modeled number headline.
